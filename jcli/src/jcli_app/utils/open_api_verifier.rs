...
pub struct OpenApiVerifier(OpenApiVerifierMode);

enum OpenApiVerifierMode {
    AcceptAll,
    Verify(OpenApiVerifierImpl),
}

struct OpenApiVerifierImpl {
    openapi: OpenAPI,
    path: String,
}
...
impl OpenApiVerifier {
    pub fn load_from_env() -> Result<Self, Error> {
        match env::var("JCLI_OPEN_API_VERIFY_PATH") {
            ...
            Err(_) => Ok(OpenApiVerifier(OpenApiVerifierMode::AcceptAll)),
        }
    }

    pub fn load(path: String) -> Result<Self, Error> {
        OpenApiVerifierImpl::load(path.clone())
            .map(|verifier| OpenApiVerifier(OpenApiVerifierMode::Verify(verifier)))
            .map_err(|source| Error::VerificationFailed { source, path })
    }

    pub fn verify_request(&self, req: &Request) -> Result<(), Error> {
        match self.0 {
            OpenApiVerifierMode::AcceptAll => Ok(()),
            OpenApiVerifierMode::Verify(ref verifier) => {
                verifier
                    .verify_request(req)
...
impl OpenApiVerifierImpl {
    ...
    pub fn load(path: String) -> Result<Self, ImplError> {
        ...
        Ok(OpenApiVerifierImpl { openapi, path })
    }
